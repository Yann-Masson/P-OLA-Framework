...
PPOTrainer::PPOTrainer(const TrainingConfig& config, const uint32_t seed)
    : _config(config)
    , _env(config, seed)
    , _rewardFn(config)
{
...
torch::Tensor PPOTrainer::normalizeState(const AIState& state) const
{
    auto tensor = torch::tensor({
        static_cast<float>(StateNorm::normalize(state.tempIn,          StateNorm::tempIn_offset,  StateNorm::tempIn_scale)),
...
void PPOTrainer::update()
{
    ...
    for (int t = N - 1; t >= 0; --t) {
        ...
        lastGAE = delta
                + static_cast<float>(_config.gamma * _config.lambda) * mask * lastGAE;
    ...
    for (int epoch = 0; epoch < _config.numEpochs; ++epoch) {
        ...
        for (int start = 0; start < N; start += _config.miniBatchSize) {
            ...
            auto surr2 = torch::clamp(ratio,
                ...
                1.0 + _config.clipEpsilon) * mbAdvantages;
            ...
            // ---- Total Loss ----
            auto totalLoss = policyLoss
                           + _config.valueCoeff * valueLoss
