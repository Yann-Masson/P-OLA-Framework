...
AIState TrainingEnvironment::reset() {
  std::cout << "[TrainingEnvironment] Resetting environment for new episode..."
            << std::endl;
  _step = 0;
...
AIState TrainingEnvironment::getState() const {
  ...
  auto userScheduleService = _provider.get<IInputService<UserScheduleData>>();
  ...
  const auto userSchedule = userScheduleService->getInput();

  return {/* tempIn            */ room->getTemperature(),
