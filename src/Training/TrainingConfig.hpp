 ...
 * @brief Configuration and hyperparameters for PPO training of the thermostat agent.
 *
 ...
 */
struct StateNorm {
    static constexpr double tempIn_offset  = 5.0,   tempIn_scale  = 30.0;   // [5, 35] °C
    static constexpr double tempOut_offset  = -20.0, tempOut_scale = 60.0;   // [-20, 40] °C
    static constexpr double price_offset   = 0.0,   price_scale   = 0.50;   // [0, 0.50] $/kWh
    ...
    static constexpr double target_offset  = 15.0,  target_scale  = 15.0;   // [15, 30] °C

    static double normalize(const double val, const double offset, const double scale) {
        return (val - offset) / (scale + 1e-8);
    }
};

/**
 * @brief Full configuration for PPO training of the smart thermostat controller.
 *
...
struct TrainingConfig {
    ...
    double entropyCoeff   = 0.01;    ///< Entropy bonus to encourage exploration
    ...
    // ---- Physics Parameters ----
    double maxHeaterPowerW        = 2000.0;    ///< Maximum heater output (W)
    double thermalCapacitance     = 500000.0;  ///< Effective room thermal mass (J/K)
    double wallConductancePerUnit = 3.75;      ///< Heat conductance per wall (W/K)
    int numWalls                  = 4;
    double windowConductance      = 7.5;       ///< Window heat conductance (W/K)
    int numWindows                = 1;
