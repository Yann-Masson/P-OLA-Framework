...
public:
   ...
   */
  template <typename TImpl> ProviderBuilder &addService() {
    ...
    deferredConstructors.push_back({typeid(TImpl).name(), [this]() {
   ...
   * @return Reference to this builder for chaining
   */
  template <typename TInterface, typename TImpl> ProviderBuilder &addService() {
   ...
   */
  Provider build() {
    // Debug log to show how many services are being constructed
    std::cout << "[ProviderBuilder] Building provider with "
              << deferredConstructors.size() << " deferred constructors"
              << std::endl;
    ...
    for (auto &constructor : deferredConstructors) {
      // Debug log before constructing each service with the name of the service
      // being constructed
      std::cout << "[ProviderBuilder] Constructing service " << index++
                << " of " << deferredConstructors.size() << " ("
...
private:
   ...
   */
  template <typename T> std::shared_ptr<T> createInstance() {
